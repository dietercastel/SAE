		...
		if(req.body["csp-report"] !== undefined){
			...
			if(options['env'] === "development" || options['env'] === "testing"){
				...
				if(originRes !== null){
					...
					tsk["uri"] = originRes[0]; 
					if( tsk["uri"].indexOf(options.useNameAsSelf) >= 0 || 
						(options.useLocalhostAsSelf && localhostRegex.test(tsk["uri"])) ) {
						//Either the name matches or 
						//Assume localhost is 'self' in a dev/test environment.
						tsk["uri"] = "'self'";
					}
